        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-10">
            ...
            <nav className="flex items-center gap-2">
              <NavTab label="홈" isActive={activeTab === 'home'} onClick={() => onTabChange('home')} />
              <NavTab label="탐색" isActive={activeTab === 'explore'} onClick={() => onTabChange('explore')} />
              <NavTab label="내 링크" isActive={activeTab === 'mylinks'} onClick={() => onTabChange('mylinks')} />
            </nav>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <IconButton
              ...
              className="p-2.5 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-white/50 dark:hover:bg-gray-800/50 rounded-lg transition-all"
            />

            {/* User Profile */}
            {session?.user && <UserProfileMenu session={session} router={router} />}
          </div>
        </div>
      </div>
    </header>
...
const NavTab = ({ label, isActive, onClick }: NavTabProps) => (
  <button
    onClick={onClick}
...
const UserProfileMenu = ({ session, router }: { session: any; router: any }) => (
