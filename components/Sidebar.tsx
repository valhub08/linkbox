...
export default function Sidebar({
  ...
  return (
    <div className="w-72 h-full overflow-y-auto flex flex-col p-5 space-y-4">
      ...
      {/* 빠른 검색 Section */}
      <section className="bg-white/40 dark:bg-gray-800/40 backdrop-blur-2xl rounded-2xl p-4 shadow-sm border border-white/30 dark:border-gray-700/30">
        ...
        <div className="space-y-1">
          <FilterButton
            ...
            icon={<IconGrid className="w-5 h-5 flex-shrink-0" />}
            label="모든 링크"
            count={totalBookmarks}
          />
          <FilterButton
            onClick={() => onSelectFilter('favorites')}
            isActive={selectedFilter === 'favorites'}
            icon={<IconStar className="w-5 h-5 flex-shrink-0" />}
            label="즐겨찾기"
          ...
          <FilterButton
            ...
            icon={<IconClock className="w-5 h-5 flex-shrink-0" />}
            label="최근 추가됨"
            count={recentCount}
          />
        </div>
                ...
                <div className="relative flex items-center">
                  ...
                  >
                    ...
                    {category.bookmarkCount > 0 && (
                      <CountBadge count={category.bookmarkCount} />
                    )}
                  </button>
                  ...
                  <IconButton
                    ...
                    icon={<IconTrash className="w-4 h-4 text-gray-500 hover:text-red-600" />}
...
const FilterButton = ({ onClick, isActive, icon, label, count }: FilterButtonProps) => (
  <button
    onClick={onClick}
...
const IconButton = ({ onClick, icon, title, className = '' }: IconButtonProps) => (
  <button
    onClick={onClick}
    className={`p-1 text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100/50 dark:hover:bg-gray-700/50 rounded-lg transition-colors ${className}`}
    title={title}
  >
    {icon}
  </button>
...
const BookmarkItem = ({ bookmark, onOpenPreview }: BookmarkItemProps) => (
  <button
  ...
  >
    <IconBookmark className="w-3 h-3 text-gray-400 flex-shrink-0" />
...
const TagButton = ({ tag, count, isSelected, onClick }: TagButtonProps) => (
  <button
    onClick={onClick}
...
const EmptyMessage = ({ message, actionLabel, onAction }: EmptyMessageProps) => (
...
const IconFolder = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
